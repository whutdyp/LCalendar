...
//                    UIImage* img =[self getImageFromUrlOrTag:elementContent];
                    if (img) {
                        img = [UIImage fitSmallImage:img];
                        NSString *lines = @"\n";
                        ...
                        NSLog(@"FTCoreTextView :%@ - Couldn't find image '(%lu,%lu)' in main bundle", self, (unsigned long)elementContentRange.location,(unsigned long)elementContentRange.length);
                        [processedString replaceCharactersInRange:tagRange withString:@"\n\n"];
                    }
                }
                else {
...
{    
	...
	for (int i = 0; i < lineCount; i++) {
        ...
        if (cfrange.location > imageNode.styleRange.location) {
			...
			CTTextAlignment alignment = kCTCenterTextAlignment;//imageNode.style.textAlignment;
			
			UIImage *img = [UIImage imageNamed:imageNode.imageName];
//            UIImage* img = [self getImageFromUrlOrTag:imageNode.imageName];
			if (img) {
                img = [UIImage fitSmallImage:img];
				int x = 0;
                ...
                UIEdgeInsets insets = imageNode.style.paragraphInset;//UIEdgeInsetsMake(5.0f, 15.0f, 5.0f, 15.0f);//
